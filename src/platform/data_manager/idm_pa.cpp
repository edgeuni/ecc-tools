  ...
  for (auto net : net_list_ptr->get_net_list()) {
    ...
    for (IdbPin* idb_pin : net->get_instance_pin_list()->get_pin_list()) {
      ...
      if (pa_list.size() > 0) {
        ...
        IEDALOG.warn(ieda::Loc::current(), "[idm error] PA do not exist for net = ", net->get_net_name(), " pin =  ", idb_pin->get_pin_name());
      }
    }
  }
...
std::vector<ids::AccessPoint> DataManager::getMasterPaPointList(std::string master_name, std::string pin_name)
{
  ...
  if (master_iter == _master_access_point_map.end()) {
    IEDALOG.warn(ieda::Loc::current(), "[idm Wraning] Can not find cell master = ", master_name);
  }
  auto pin_iter = (master_iter->second).find(pin_name);
  if (pin_iter == (master_iter->second).end()) {
    IEDALOG.warn(ieda::Loc::current(), "[idm Wraning] Can not find pin = ", pin_name, " in cell master = ", master_name);
    return {};
  }
  ...
  if (idb_inst != nullptr) {
    ...
    for (auto& pa : access_point_list) {
      ...
      if (pa.x > inst_boundary->get_high_x() || pa.x < inst_boundary->get_low_x() || pa.y > inst_boundary->get_high_y()
        ...
        IEDALOG.warn(ieda::Loc::current(), "[idm error] PA x = ", pa.x, " y = ", pa.y, " is not inside the instance ", idb_inst->get_name(), " ( ", inst_boundary->get_low_x(), ", ", inst_boundary->get_low_y(), ") ( ", inst_boundary->get_high_x(), ", ", inst_boundary->get_high_y(), ") ");
      }
    }
  }
