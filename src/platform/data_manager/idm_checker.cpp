...
bool DataManager::isOnIOSite(int32_t llx, int32_t lly, int32_t urx, int32_t ury, IdbOrient orient)
{
  ...
  if (orient == IdbOrient::kE_R270 || orient == IdbOrient::kW_R90) {
    ...
    if (y_to_bottom % site_width != 0) {
      IEDALOG.warn(ieda::Loc::current(), "IO cell does not match IO site.");
      return false;
    }
...
{
  IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Begin check connected ...");
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (auto disconnected_net : pin_num_disconnected_net_map[2]) {
    IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Disconneted net [pin number == 2] : ", disconnected_net);
  }
  for (auto disconnected_net : pin_num_disconnected_net_map[3]) {
    IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Disconneted net [pin number >= 3] : ", disconnected_net);
  }
  if (!pin_num_disconnected_net_map[0].empty()) {
    IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Disconneted net [pin number == 0] : ", pin_num_disconnected_net_map[0].size(), " / ", _design->get_net_list()->get_net_list().size());
  }
  if (!pin_num_disconnected_net_map[1].empty()) {
    IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Disconneted net [pin number == 1] : ", pin_num_disconnected_net_map[1].size(), " / ", _design->get_net_list()->get_net_list().size());
  }
  if (!pin_num_disconnected_net_map[2].empty()) {
    IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Disconneted net [pin number == 2] : ", pin_num_disconnected_net_map[2].size(), " / ", _design->get_net_list()->get_net_list().size());
  }
  if (!pin_num_disconnected_net_map[3].empty()) {
    IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] Disconneted net [pin number >= 3] : ", pin_num_disconnected_net_map[3].size(), " / ", _design->get_net_list()->get_net_list().size());
  }
  ...
  IEDALOG.info(ieda::Loc::current(), "[CheckNet Info] End check connected");
