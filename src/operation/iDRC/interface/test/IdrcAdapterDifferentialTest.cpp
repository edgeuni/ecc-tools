...
struct ShapeKey
{
  ...
  {
    return std::make_tuple(net_idx, layer_idx, is_routing, ll_x, ll_y, ur_x, ur_y);
  }
...
std::vector<ShapeKey> canonicalShapes(const std::vector<ids::Shape>& shapes)
{
  std::vector<ShapeKey> keys;
  keys.reserve(shapes.size());
  for (const auto& shape : shapes) {
    keys.push_back(ShapeKey{.net_idx = shape.net_idx,
...
std::string shapeToString(const ShapeKey& shape)
{
  ...
  out << "net=" << shape.net_idx << " layer=" << shape.layer_idx << " routing=" << (shape.is_routing ? "true" : "false") << " rect=("
      << shape.ll_x << ',' << shape.ll_y << ',' << shape.ur_x << ',' << shape.ur_y << ')';
  return out.str();
}

void expectShapeListsEqual(std::string_view tag, const std::vector<ids::Shape>& left, const std::vector<ids::Shape>& right)
{
  const auto left_keys = canonicalShapes(left);
  const auto right_keys = canonicalShapes(right);
...
TEST(Shapes, GeneratedViaAndRegularWireMatch)
{
  ...
  expectShapeListsEqual("env", idb_env, entt_env);
  expectShapeListsEqual("result", idb_result, entt_result);
  const auto result_keys = canonicalShapes(idb_result);
  EXPECT_NE(std::find(result_keys.begin(), result_keys.end(),
                      ShapeKey{.net_idx = 0, .layer_idx = 0, .is_routing = true, .ll_x = 50, .ll_y = 250, .ur_x = 550, .ur_y = 350}),
            result_keys.end())
      << "a_sig should use canonical net_idx 0";
  EXPECT_NE(std::find(result_keys.begin(), result_keys.end(),
                      ShapeKey{.net_idx = 1, .layer_idx = 0, .is_routing = true, .ll_x = 50, .ll_y = 50, .ur_x = 550, .ur_y = 150}),
            result_keys.end())
      << "z_sig should use canonical net_idx 1";
