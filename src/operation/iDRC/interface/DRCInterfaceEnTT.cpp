...
class EnttLayerTable
{
 public:
  ...
  [[nodiscard]] int32_t routingWrapIdx(idb::refactor::TechRoutingLayerId layer) const
  {
    const auto* index = find(idb::refactor::TechLayerId{layer.entity()});
...
ViaShapes generatedViaShapes(const idb::refactor::DesignVia& via, idb::refactor::Point origin)
{
  ...
  const int32_t origin_x = (generated.flags & idb::refactor::DesignGeneratedViaFlag::kHasOrigin) != 0 ? generated.origin.x : 0;
  const int32_t origin_y = (generated.flags & idb::refactor::DesignGeneratedViaFlag::kHasOrigin) != 0 ? generated.origin.y : 0;
...
idb::refactor::Rect enlargedWireSegmentBox(idb::refactor::Point a, idb::refactor::Point b, int32_t half_width)
{
  return idb::refactor::Rect{.ll_x = std::min(a.x, b.x),
                             .ll_y = std::min(a.y, b.y),
                             .ur_x = std::max(a.x, b.x),
...
std::vector<idb::refactor::DesignNetId> canonicalRegularNets(const idb::refactor::DesignDatabase& design)
{
...
std::vector<idb::refactor::DesignNetId> canonicalSpecialNets(const idb::refactor::DesignDatabase& design)
{
...
NetSkipInfo analyzeNetSkip(const idb::refactor::DesignDatabase& design, const idb::refactor::LibraryDatabase& library,
...
bool netIsSkipping(const idb::refactor::DesignDatabase& design, const idb::refactor::LibraryDatabase& library, idb::refactor::DesignNetId net,
                   NetSkipCache& cache)
{
  if (!net) {
    return true;
...
NetIndexMap canonicalNetIndexMap(const idb::refactor::DesignDatabase& design)
{
  NetIndexMap map;
  int32_t index = 0;
  for (const auto net_id : canonicalRegularNets(design)) {
    map.emplace(static_cast<uint32_t>(net_id.packed()), index++);
  }
  for (const auto net_id : canonicalSpecialNets(design)) {
    map.emplace(static_cast<uint32_t>(net_id.packed()), index++);
  }
  return map;
}

int32_t netIndex(idb::refactor::DesignNetId net, const NetIndexMap& net_index_map)
{
  if (!net) {
    return -1;
  }
...
void appendShape(std::vector<ids::Shape>& shape_list, const EnttLayerTable& layers, idb::refactor::TechLayerId layer,
                 idb::refactor::Rect rect, int32_t net_idx)
{
...
{
  std::vector<std::pair<idb::refactor::TechLayerId, std::vector<idb::refactor::Rect>>> routing_by_layer;
  for (const auto& shape : shapes.routing) {
...
void collectPortGeometry(const idb::refactor::LibraryDatabase& library, const idb::refactor::TechDatabase& tech,
                         const EnttLayerTable& layers, const idb::refactor::LibraryMasterPort& port, idb::refactor::Point origin,
                         idb::refactor::DesignOrientation orient, int32_t width, int32_t height, std::vector<LayeredRect>& routing,
...
void wrapTrackAxisFromEnTT(RoutingLayer& routing_layer, const idb::refactor::TechRoutingLayer& tech_layer,
                           idb::refactor::TechRoutingLayerId layer_id, const idb::refactor::DesignDatabase& design)
{
  const bool prefer_horizontal = tech_layer.direction == idb::refactor::TechRoutingDirection::kHorizontal;
  const auto wanted = prefer_horizontal ? idb::refactor::DesignAxis::kY : idb::refactor::DesignAxis::kX;
                                   ...
                                   const idb::refactor::TechDatabase& tech)
{
...

  {
    ...
    if (!rules.empty()) {
      ...
      for (const auto rule_id : rules) {
        ...
        corner.has_convex_corner = src.type == idb::refactor::TechRoutingLef58CornerType::kConvex;
        corner.has_concave_corner = src.type == idb::refactor::TechRoutingLef58CornerType::kConcave;
        corner.has_except_eol = (src.flags & idb::refactor::TechRoutingLef58CornerSpacingRuleFlag::kHasExceptEol) != 0;
...

  {
    ...
    if (!eol_ids.empty()) {
      ...
      for (const auto eol_id : eol_ids) {
        ...
        eol.has_ete = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kHasEndToEnd) != 0;
        if (eol.has_ete) {
          eol.ete_spacing = src.end_to_end_space;
        }
        eol.has_par = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kHasParallelEdge) != 0;
        if (eol.has_par) {
          eol.has_subtrace_eol_width = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kSubtractEolWidth) != 0;
          eol.par_spacing = src.parallel_space;
          eol.par_within = src.parallel_within;
          eol.has_two_edges = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kTwoEdges) != 0;
          eol.has_min_length = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kHasParallelMinLength) != 0;
          if (eol.has_min_length) {
            eol.min_length = src.parallel_min_length;
          }
          eol.has_same_metal = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kSameMetal) != 0;
        }
        eol.has_enclose_cut = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kHasEncloseCut) != 0;
        if (eol.has_enclose_cut) {
          eol.has_below = src.enclose_cut_side == idb::refactor::CutLayerSide::kBelow;
          eol.has_above = src.enclose_cut_side == idb::refactor::CutLayerSide::kAbove;
          eol.enclosed_dist = src.enclose_distance;
          eol.cut_to_metal_spacing = src.cut_to_metal_spacing;
          eol.has_all_cuts = (src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kAllCuts) != 0;
        }
    ...
    if (!native_eol_ids.empty()) {
      ...
      for (const auto eol_id : native_eol_ids) {
        ...
        eol.eol_width = src.eol_width;
        eol.eol_within = src.eol_within;
        eol.has_par = (src.flags & idb::refactor::TechRoutingEndOfLineSpacingRuleFlag::kHasParallelEdge) != 0;
        if (eol.has_par) {
          eol.par_spacing = src.parallel_space;
          eol.par_within = src.parallel_within;
          eol.has_two_edges = (src.flags & idb::refactor::TechRoutingEndOfLineSpacingRuleFlag::kTwoEdges) != 0;
        }
        eol_list.push_back(eol);
      }
      exist_rule_set.insert(ViolationType::kEndOfLineSpacing);
...

  {
    ...
    maximum_width.max_width = (tech_layer.flags & idb::refactor::TechRoutingLayerFlag::kHasMaxWidth) != 0 ? tech_layer.max_width
...

  {
    ...
    if (!mincut_ids.empty()) {
      ...
      for (const auto mincut_id : mincut_ids) {
        ...
        MinimumCutRule rule;
        if ((src.flags & idb::refactor::TechRoutingLef58MinimumCutRuleFlag::kHasNumCuts) != 0) {
        ...
        rule.has_within_cut_distance = (src.flags & idb::refactor::TechRoutingLef58MinimumCutRuleFlag::kHasWithinCutDistance) != 0;
        if (rule.has_within_cut_distance) {
          rule.within_cut_distance = src.within_cut_distance;
        }
        rule.has_from_above = src.orient == idb::refactor::TechRoutingMinimumCutOrient::kFromAbove;
        rule.has_from_below = src.orient == idb::refactor::TechRoutingMinimumCutOrient::kFromBelow;
        rule.has_length = (src.flags & idb::refactor::TechRoutingLef58MinimumCutRuleFlag::kHasLength) != 0;
        if (rule.has_length) {
...

  {
    ...
    for (const auto spacing_id : spacing_ids) {
      ...
      const auto type = src.type == idb::refactor::TechRoutingSpacingType::kRange ? LayerSpacingType::kSpacingRange
...
void wrapCutDesignRuleFromEnTT(CutLayer& cut_layer, idb::refactor::TechCutLayerId layer_id, const idb::refactor::TechDatabase& tech)
{
...

  {
    ...
    const idb::refactor::TechCutLef58SpacingTableRule* second_layer_table = nullptr;
...

  {
    ...
    if (!edge_ids.empty()) {
      ...
      for (const auto edge_id : edge_ids) {
        ...
        if ((src.flags & idb::refactor::TechCutLef58EnclosureEdgeRuleFlag::kHasConvexCorners) != 0) {
          ...
          rule.has_above = src.side == idb::refactor::CutLayerSide::kAbove;
          rule.has_below = src.side == idb::refactor::CutLayerSide::kBelow;
        ...
        rule.has_except_two_edges = (src.flags & idb::refactor::TechCutLef58EnclosureEdgeRuleFlag::kExceptTwoEdges) != 0;
        edge_list.push_back(rule);
      }
...

  {
    ...
    if (eol_id) {
      ...
      rule.has_below = src.side == idb::refactor::CutLayerSide::kBelow;
      rule.overhang = src.overhang;
      if ((src.flags & idb::refactor::TechCutLef58EolEnclosureRuleFlag::kHasParallelSpace) != 0) {
        rule.par_spacing = src.parallel_space;
      }
      if ((src.flags & idb::refactor::TechCutLef58EolEnclosureRuleFlag::kHasExtension) != 0) {
        rule.backward_ext = src.backward_ext;
        rule.forward_ext = src.forward_ext;
      }
      rule.has_min_length = (src.flags & idb::refactor::TechCutLef58EolEnclosureRuleFlag::kHasMinLength) != 0;
...

  {
    ...
    for (const auto spacing_id : storage.spacingRules(layer_id)) {
      ...
      if ((src.flags & idb::refactor::TechCutSpacingRuleFlag::kHasAdjacentCuts) == 0) {
        continue;
      }
...

  {
    ...
    if (!spacing_ids.empty()) {
      for (const auto spacing_id : spacing_ids) {
        ...
        if ((spacing_rule.flags & idb::refactor::TechCutSpacingRuleFlag::kHasAdjacentCuts) != 0) {
          continue;
        }
        same.spacings.push_back({spacing_rule.spacing,
                                 -1,
                                 -1,
                                 (spacing_rule.flags & idb::refactor::TechCutSpacingRuleFlag::kSameNet) != 0});
      }
...
std::vector<ids::Shape> DRCInterface::buildEnvShapeListFromEnTT()
{
  ...
  const EnttLayerTable layers(tech);
  const NetIndexMap net_index_map = canonicalNetIndexMap(design);
    ...
    for (const auto wire_id : design.routingStorage().wires(net_id)) {
      ...
      for (std::size_t path_index = 0; path_index < path_count; ++path_index) {
        ...
        if (points.size() >= 2) {
          ...
          appendShape(result_shape_list, layers, idb::refactor::TechLayerId{path.layer().entity()},
        ...
        for (const auto& rectangle : path.rectangles()) {
          ...
          appendShape(result_shape_list, layers, idb::refactor::TechLayerId{path.layer().entity()}, rectangle.delta.offset(base.x, base.y),
                      net_idx);
        }
      }
    }
    ...
    for (const auto wire_id : design.routingStorage().wires(net_id)) {
      ...
      for (std::size_t path_index = 0; path_index < path_count; ++path_index) {
        ...
        if (path.vias().empty()) {
          ...
          appendShape(result_shape_list, layers, idb::refactor::TechLayerId{path.layer().entity()}, rectangle, net_idx);
        }
        for (const auto via : path.vias()) {
          if (via.point_index >= points.size()) {
            continue;
