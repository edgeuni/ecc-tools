...
class RCTable
{
 public:
  ...
  void init(int32_t corner_num, int32_t net_num, TopoPool& topo_pool)
  {
    ...
    _net_ccap_entry_list.assign(static_cast<size_t>(_net_num), std::vector<CcapEntry>());
  ...
  void append_net_ccap_entry(int32_t net_idx, int32_t first_edge_idx, int32_t second_edge_idx, int32_t corner_idx, double cap)
  {
    _net_ccap_entry_list.at(static_cast<size_t>(net_idx)).emplace_back(first_edge_idx, second_edge_idx, corner_idx, cap);
  }
    ...
    for (std::vector<CcapEntry>& ccap_entry_list : _net_ccap_entry_list) {
      for (CcapEntry& ccap_entry : ccap_entry_list) {
        ...
        if (_merged_ccap_map.count(coupling_key) == 0) {
          _merged_ccap_map[coupling_key].resize(static_cast<size_t>(_corner_num), 0.0);
        }
        _merged_ccap_map[coupling_key].at(static_cast<size_t>(ccap_entry.get_corner_idx())) += ccap_entry.get_cap();
      }
