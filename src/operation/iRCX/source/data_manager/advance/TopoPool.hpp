...
class TopoPool
{
 public:
  ...
  TopoNode& get_node(int32_t node_idx) { return _node_pool.at(static_cast<size_t>(node_idx)); }
  TopoEdge& get_edge(int32_t edge_idx) { return _edge_pool.at(static_cast<size_t>(edge_idx)); }
  std::span<TopoNode> get_net_node_list(int32_t net_id)
  {
    std::pair<size_t, size_t> net_node_range = _net_node_range_list.at(static_cast<size_t>(net_id));
    return std::span<TopoNode>(_node_pool.data() + net_node_range.first, net_node_range.second);
  }
  std::span<TopoEdge> get_net_edge_list(int32_t net_id)
  {
    std::pair<size_t, size_t> net_edge_range = _net_edge_range_list.at(static_cast<size_t>(net_id));
    return std::span<TopoEdge>(_edge_pool.data() + net_edge_range.first, net_edge_range.second);
  }
  std::pair<size_t, size_t> get_net_node_range(int32_t net_id) { return _net_node_range_list.at(static_cast<size_t>(net_id)); }
  std::pair<size_t, size_t> get_net_edge_range(int32_t net_id) { return _net_edge_range_list.at(static_cast<size_t>(net_id)); }
  // setter
  // function
  int32_t get_node_idx(int32_t net_id, int32_t local_node_idx)
  {
    return static_cast<int32_t>(_net_node_range_list.at(static_cast<size_t>(net_id)).first + static_cast<size_t>(local_node_idx));
  }
  int32_t get_edge_idx(int32_t net_id, int32_t local_edge_idx)
  {
    return static_cast<int32_t>(_net_edge_range_list.at(static_cast<size_t>(net_id)).first + static_cast<size_t>(local_edge_idx));
  }
 ...
 private:
  void assign_node_id(std::vector<TopoNode>& node_list)
  {
    for (int32_t node_idx = 0; node_idx < static_cast<int32_t>(node_list.size()); ++node_idx) {
      node_list[static_cast<size_t>(node_idx)].set_node_id(node_idx);
    }
  }
  void assign_edge_id(std::vector<TopoEdge>& edge_list, bool is_special_net)
  {
    for (int32_t edge_idx = 0; edge_idx < static_cast<int32_t>(edge_list.size()); ++edge_idx) {
      edge_list[static_cast<size_t>(edge_idx)].set_edge_id(edge_idx);
      edge_list[static_cast<size_t>(edge_idx)].set_is_special_net(is_special_net);
    }
  }
