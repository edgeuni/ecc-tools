...
class LayoutData
{
 public:
  ...
  GtlRectI& get_die_shape() { return _die_shape; }
  Dbu get_dbu_per_micron() const { return _dbu_per_micron; }
  std::map<Size, RoutingLayer>& get_routing_layer_map() { return _routing_layer_map; }
  ...
  void set_die_shape(const GtlRectI& die_shape) { _die_shape = die_shape; }
  void set_dbu_per_micron(Dbu dbu_per_micron) { _dbu_per_micron = dbu_per_micron; }
  void set_routing_layer_map(const std::map<Size, RoutingLayer>& routing_layer_map) { _routing_layer_map = routing_layer_map; }
  ...
  // function
  Size get_regular_net_count() const { return _net_list.size(); }
  bool get_is_empty() const;

 private:
  std::string _design_name;
  GtlRectI _die_shape;
  Dbu _dbu_per_micron = 1;
  std::map<Size, RoutingLayer> _routing_layer_map;
  std::vector<Net> _net_list;
  Net _special_net;
};

inline bool LayoutData::get_is_empty() const
{
