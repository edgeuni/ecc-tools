...
void ResistanceCalculator::initInst()
{
  if (_rc_instance == nullptr) {
    _rc_instance = new ResistanceCalculator();
  }
}

ResistanceCalculator& ResistanceCalculator::getInst()
{
  if (_rc_instance == nullptr) {
...
void ResistanceCalculator::destroyInst()
{
  if (_rc_instance != nullptr) {
    delete _rc_instance;
...
void ResistanceCalculator::calculate()
{
  Monitor monitor;
  RCXLOG.info(Loc::current(), "Starting...");

  RCModel rc_model = initRCModel();
  calculateRCModel(rc_model);
...
ResistanceCalculator* ResistanceCalculator::_rc_instance = nullptr;

RCModel ResistanceCalculator::initRCModel()
{
  RCModel rc_model;
  rc_model.set_database(&RCXDM.getDatabase());
  rc_model.set_temp_directory_path(RCXDM.getConfig().rc_temp_directory_path);
  return rc_model;
}

void ResistanceCalculator::calculateRCModel(RCModel&)
{
  calculateResistance();
}

void ResistanceCalculator::calculateResistance()
{
  ...
  for (Size corner_idx = 0; corner_idx < corner_num; corner_idx++) {
    calculateCornerResistance(corner_idx);
  }
}

void ResistanceCalculator::calculateCornerResistance(Size corner_idx)
{
...
#pragma omp parallel for schedule(dynamic) num_threads(thread_num)
  for (Size net_idx = 0; net_idx < net_num; net_idx++) {
    calculateNetResistance(corner_idx, net_idx);
  }
}

void ResistanceCalculator::calculateNetResistance(Size corner_idx, Size net_idx)
{
  Database& database = RCXDM.getDatabase();
  ...
  for (Size edge_idx = 0; edge_idx < edge_list.size(); edge_idx++) {
    ...
    if (edge.get_is_via()) {
      ...
      if (via != nullptr) {
        resistance_list[edge_idx] = calculateViaResistance(corner_data, *via, edge);
      }
      continue;
    }
    ...
    resistance_list[edge_idx] = calculateWireResistance(corner_data, *conductor, edge, edge_interval_list);
  }
}

F64 ResistanceCalculator::calculateWireResistance(CornerData& corner_data,
                                                   ...
                                                   std::span<EdgeEtchInterval> edge_interval_list)
{
  ...
  Micron segment_start = edge.get_line_segment().get_is_horizontal() ? boost::polygon::x(start_node.get_point()) * micron_per_dbu
                                                                      : boost::polygon::y(start_node.get_point()) * micron_per_dbu;
  Micron segment_end = edge.get_line_segment().get_is_horizontal() ? boost::polygon::x(end_node.get_point()) * micron_per_dbu
  ...
  for (EdgeEtchInterval& edge_interval : edge_interval_list) {
    Micron overlap_start = std::max(edge_interval.get_start_coordinate(), segment_start);
    Micron overlap_end = std::min(edge_interval.get_end_coordinate(), segment_end);
    if (overlap_end <= overlap_start) {
      continue;
    }

    Micron length = overlap_end - overlap_start;
    Micron width = edge_interval.get_width();
    Micron thickness = edge_interval.get_thickness();
...
F64 ResistanceCalculator::calculateViaResistance(CornerData& corner_data, ProcessVia& via, TopoEdge& edge)
{
  Database& database = RCXDM.getDatabase();
  Micron micron_per_dbu = unit::to_micron(1, database.get_layout_data().get_dbu_per_micron());
  GtlRectI& via_shape = edge.get_shape();
  Micron x_span = (boost::polygon::xh(via_shape) - boost::polygon::xl(via_shape)) * micron_per_dbu;
  Micron y_span = (boost::polygon::yh(via_shape) - boost::polygon::yl(via_shape)) * micron_per_dbu;
  Micron length = std::max(x_span, y_span) * corner_data.get_half_node_scale_factor();
  Micron width = std::min(x_span, y_span) * corner_data.get_half_node_scale_factor();
  std::pair<Micron, Micron> etch_pair = via.query_etch(ProcessEffectType::kResistance, width, length);
  length = std::max<Micron>(0.0, length - 2.0 * etch_pair.first);
  width = std::max<Micron>(0.0, width - 2.0 * etch_pair.second);
...
F64 ResistanceCalculator::getTemperatureFactor(F64 temperature,
...
ProcessVia* ResistanceCalculator::getProcessVia(CornerData& corner_data, Size design_layer_id)
{
...
ProcessConductor* ResistanceCalculator::getProcessConductor(CornerData& corner_data, Size design_layer_id)
{
  LayerTable& layer_table = RCXDM.getDatabase().get_layer_table();
  std::unordered_map<Size, std::string>& design_id_to_name_map = layer_table.get_design_id_to_name_map();
  if (design_id_to_name_map.count(design_layer_id) == 0) {
