...
class EnvTrackOverlapMerge
{
 ...
 private:
  static EnvTrackOverlap makeNullOverlap(int32_t a0,
                                      int32_t a1)
  {
    EnvTrackOverlap ov;
    ov.a0 = a0;
  ...
  static void emitTrackOverlap(std::vector<EnvTrackOverlap>& out,
                               const EnvTrackOverlap& ov)
  {
    if (!(ov.a0 < ov.a1)) {
      return;
    }
...
{
 public:
  struct EnvLayerPixelOverlaps {
    size_t layer = 0;                 // 0 means invalid / absent
    std::vector<EnvPixelOverlap> segs; // higher priority comes first in input order
  };
               ...
               std::vector<CrossOverlapSub>& out) const
  {
    ...
    std::vector<EnvNormalizedLayerPixelOverlaps> dn_norm;
    std::vector<EnvNormalizedLayerPixelOverlaps> up_norm;
    ...
    for (const auto& in : dn_inputs) {
      ...
      EnvNormalizedLayerPixelOverlaps ni;
      ni.layer = in.layer;
 ...
 private:
  struct EnvNormalizedLayerPixelOverlaps {
    size_t layer = 0;
    std::vector<EnvPixelOverlap> segs;
  };

  static size_t countBreakpoints(const std::vector<EnvNormalizedLayerPixelOverlaps>& inputs)
  {
    size_t n = 0;
  ...
  static void addBreakpoints(const std::vector<EnvNormalizedLayerPixelOverlaps>& inputs,
                             std::vector<int32_t>& bp)
  {
    for (const auto& in : inputs) {
                           ...
                           std::vector<EnvPixelOverlap>& out)
  {
    ...
    for (const auto& ov : in) {
      ...
      const int32_t a1 = std::min(query_a1, ov.a1);
      if (a0 < a1) {
        out.push_back(EnvPixelOverlap{a0, a1});
      }
    }
  ...
  static void advanceCursor(const std::vector<EnvPixelOverlap>& segs,
                            size_t& idx,
                            int32_t x)
  {
  ...
  static bool covers(const std::vector<EnvPixelOverlap>& segs,
                     size_t idx,
                     int32_t a0,
                     int32_t a1)
  {
  ...
  static size_t firstCoveringLayer(const std::vector<EnvNormalizedLayerPixelOverlaps>& inputs,
