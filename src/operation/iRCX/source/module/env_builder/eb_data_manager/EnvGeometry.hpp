...
inline auto manhattanDistance(const P& a,
                               const P& b)
    -> decltype(std::abs(env_geom::x(a) - env_geom::x(b)) + std::abs(env_geom::y(a) - env_geom::y(b))) {
  const auto dx = std::abs(env_geom::x(a) - env_geom::x(b));
  const auto dy = std::abs(env_geom::y(a) - env_geom::y(b));
  return dx + dy;
}
...
inline auto isHorizontalDominant(const P& a,
                                   const P& b) -> bool
{
  return std::abs(env_geom::x(a) - env_geom::x(b)) >= std::abs(env_geom::y(a) - env_geom::y(b));
}
...
inline auto isLowerLeft(const P& a,
                          const P& b) -> bool
{
  return env_geom::x(a) <= env_geom::x(b) && env_geom::y(a) <= env_geom::y(b);
}
...
inline auto rectContainsPoint(const R& r,
                                const P& p) -> bool
{
  return env_geom::x(p) >= minX(r) && env_geom::x(p) <= maxX(r) &&
         env_geom::y(p) >= minY(r) && env_geom::y(p) <= maxY(r);
}
...
inline auto translatePoint(const P& p,
  ...
  using PP = remove_cvref_t<P>;
  return makePoint<PP>(env_geom::x(p) + dx, env_geom::y(p) + dy);
}
...
inline auto rectAround(const P& p,
  ...
  return gtl::rectangle_data<T>{env_geom::x(p) - d, env_geom::y(p) - d, env_geom::x(p) + d, env_geom::y(p) + d};
}
...
inline auto rectRelation(T a_llx,
                          ...
                          T b_lly,
                          T b_urx,
                          T b_ury) -> EnvRectRelation<T>
{
  EnvRectRelation<T> relation;
