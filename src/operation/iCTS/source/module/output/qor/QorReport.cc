...
auto QorReport::write(const EvaluationState& evaluation_state, const std::string& statistics_dir) -> bool
{
  ...
  bool success = true;
  success &= writeReportFile(statistics_path / "wirelength.rpt", "Wirelength Statistics",
                             {"Metric", "Routed Wirelength (um)", "HPWL Wirelength (um)"}, buildWirelengthReportRows(statistics));
  success &= writeReportFile(statistics_path / "cell_stats.rpt", "Cell Stats",
                             {"Cell Type", "Count", "Total Area (um^2)", "Total Cap (pF)"}, buildCellStatsReportRows(statistics));
  success &= writeReportFile(statistics_path / "lib_cell_dist.rpt", "Library Cell Distribution",
