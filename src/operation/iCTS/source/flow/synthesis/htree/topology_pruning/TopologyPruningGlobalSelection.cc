...
{
  LOG_FATAL_IF(front.empty()) << "HTree: empty global selection front.";
  double front_min_delay = front.front().entry->get_delay();
  for (const auto& entry_ref : front) {
    LOG_FATAL_IF(entry_ref.entry == nullptr) << "HTree: null global selection front entry.";
                         ...
                         const BufferPatternLibrary& segment_pattern_library) -> std::size_t
{
  ...
  for (const auto segment_pattern_id : topology_pattern.get_level_segment_pattern_ids()) {
    ...
    LOG_FATAL_IF(segment_pattern == nullptr) << "HTree: missing segment pattern while comparing global candidate shape.";
...
{
  LOG_FATAL_IF(entry_ref.entry == nullptr) << "HTree: null global candidate while comparing shape.";
  LOG_FATAL_IF(entry_ref.candidate_index >= evaluations.size()) << "HTree: global candidate index exceeds evaluated depth count.";
...
{
  LOG_FATAL_IF(front.empty()) << "HTree: invalid empty global selection decision front.";
