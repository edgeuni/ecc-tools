...
auto expectCanonicalTableInLog(const std::vector<std::string>& report_lines, const std::vector<std::string>& log_messages,
                               const std::string& title) -> void
{
  ASSERT_FALSE(report_lines.empty());
  const auto title_line
      = std::ranges::find_if(report_lines, [&title](const std::string& line) -> bool { return line.find(title) != std::string::npos; });
