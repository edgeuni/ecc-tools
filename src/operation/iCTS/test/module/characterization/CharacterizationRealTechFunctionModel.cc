    ...
    -> std::unordered_map<std::string, StructuralCapOperator>
{
  ...
  for (const auto& [pattern_id, pattern] : segment_context.patterns) {
    ...
    double alpha = 1.0;
    double eta_pf = CTSDM.getWrapper().queryWireCapacitance(routing_layer, unit_length_um, config.wire_width_um);
    if (!cell_masters.empty()) {
      ...
      eta_pf = CTSDM.getWrapper().queryCharInputPinCap(cell_masters.front());
      eta_pf += CTSDM.getWrapper().queryWireCapacitance(routing_layer, prewire_length_um, config.wire_width_um);
    }
