...
struct WrapperEntityTables
{
  ...
  EntityPointerTable<idb::refactor::DesignInstance, idb::refactor::DesignRegistry::registry_type> instances;
  EntityPointerTable<idb::refactor::DesignInstancePin, idb::refactor::DesignRegistry::registry_type> instance_pins;
  EntityPointerTable<idb::refactor::DesignIoPin, idb::refactor::DesignRegistry::registry_type> io_pins;
  EntityPointerTable<idb::refactor::DesignNet, idb::refactor::DesignRegistry::registry_type> nets;
  EntityPointerTable<idb::refactor::LibraryCellMaster, idb::refactor::LibraryRegistry::registry_type> masters;
  EntityPointerTable<idb::refactor::LibraryMasterObs, idb::refactor::LibraryRegistry::registry_type> master_obs;
  EntityPointerTable<idb::refactor::LibraryMasterTerm, idb::refactor::LibraryRegistry::registry_type> master_terms;
  EntityPointerTable<idb::refactor::LibraryMasterPort, idb::refactor::LibraryRegistry::registry_type> master_ports;
};

class EnttLayerTable
{
 public:
  explicit EnttLayerTable(const idb::refactor::TechDatabase& tech)
  {
    ...
    for (int32_t order = 0; order < static_cast<int32_t>(sequence.size()); ++order) {
      ...
      index.order = order;
      if (tech.routingLayerStorage().contains(idb::refactor::TechRoutingLayerId{layer.entity()})) {
        index.routing = true;
        index.wrap_idx = routing_idx++;
      } else if (tech.cutLayerStorage().contains(idb::refactor::TechCutLayerId{layer.entity()})) {
        index.cut = true;
  ...
  [[nodiscard]] const EnttLayerIndex* find(idb::refactor::TechLayerId layer) const
  {
    if (!layer) {
      return nullptr;
    }
...
Direction directionFromTech(idb::refactor::TechRoutingDirection direction)
{
  switch (direction) {
    case idb::refactor::TechRoutingDirection::kHorizontal:
      return Direction::kHorizontal;
    case idb::refactor::TechRoutingDirection::kVertical:
...
ConnectType connectTypeFromUse(idb::refactor::DesignSignalUse use)
{
  return use == idb::refactor::DesignSignalUse::kClock ? ConnectType::kClock : ConnectType::kSignal;
}

bool isCoreMaster(idb::refactor::LibraryCellMasterType type)
{
  return type >= idb::refactor::LibraryCellMasterType::kCore && type <= idb::refactor::LibraryCellMasterType::kEndcapBottomRight;
}

bool isPadMaster(idb::refactor::LibraryCellMasterType type)
{
  return type >= idb::refactor::LibraryCellMasterType::kPad && type <= idb::refactor::LibraryCellMasterType::kPadAreaIo;
}

void transformCoordinate(int32_t& x, int32_t& y, idb::refactor::DesignOrientation orient, idb::refactor::Point origin, int32_t width,
                         int32_t height)
{
  switch (orient) {
    case idb::refactor::DesignOrientation::kN:
      return;
    case idb::refactor::DesignOrientation::kW: {
    ...
    case idb::refactor::DesignOrientation::kS: {
      const int32_t dx = x - origin.x;
      const int32_t dy = y - origin.y;
      x = -dx + width + origin.x;
    ...
    case idb::refactor::DesignOrientation::kE: {
      const int32_t dx = x - origin.x;
      const int32_t dy = y - origin.y;
      x = dy + origin.x;
      y = -dx + width + origin.y;
    ...
    case idb::refactor::DesignOrientation::kFN: {
      const int32_t dx = x - origin.x;
      const int32_t dy = y - origin.y;
      x = -dx + width + origin.x;
      y = dy + origin.y;
    ...
    case idb::refactor::DesignOrientation::kFS: {
      const int32_t dx = x - origin.x;
      const int32_t dy = y - origin.y;
      x = dx + origin.x;
      y = -dy + height + origin.y;
    ...
    case idb::refactor::DesignOrientation::kFW: {
      const int32_t dx = x - origin.x;
      const int32_t dy = y - origin.y;
      x = dy + origin.x;
      y = dx + origin.y;
    ...
    case idb::refactor::DesignOrientation::kFE: {
      const int32_t dx = x - origin.x;
      const int32_t dy = y - origin.y;
      x = -dy + height + origin.x;
      y = -dx + width + origin.y;
...
idb::refactor::Rect transformRect(idb::refactor::Rect rect, idb::refactor::DesignOrientation orient, idb::refactor::Point origin,
                                  int32_t width, int32_t height)
{
  ...
  return idb::refactor::Rect{.ll_x = std::min(ll_x, ur_x),
                             .ll_y = std::min(ll_y, ur_y),
                             .ur_x = std::max(ll_x, ur_x),
                             .ur_y = std::max(ll_y, ur_y)};
}

idb::refactor::Rect placeMasterRect(idb::refactor::Rect local, idb::refactor::Point origin, idb::refactor::DesignOrientation orient,
                                    int32_t width, int32_t height)
{
  return transformRect(local.offset(origin.x, origin.y), orient, origin, width, height);
}

idb::refactor::Point placeMasterPoint(idb::refactor::Point local, idb::refactor::Point origin, idb::refactor::DesignOrientation orient,
                                      int32_t width, int32_t height)
{
  idb::refactor::Point placed{local.x + origin.x, local.y + origin.y};
  transformCoordinate(placed.x, placed.y, orient, origin, width, height);
  return placed;
}

std::vector<idb::refactor::Rect> geometryRects(const idb::refactor::GeometryPool& pool, idb::refactor::GeometryHandle handle)
{
  std::vector<idb::refactor::Rect> rects;
  ...
  for (uint32_t index = 0; index < pool.polygonCount(handle); ++index) {
    ...
    auto decomposed = idb::refactor::decomposePolygonToRectangles(points);
...
void forEachGeometryRect(const idb::refactor::GeometryPool& pool, idb::refactor::GeometryHandle handle, Function&& function)
{
  ...
  for (uint32_t index = 0; index < pool.polygonCount(handle); ++index) {
    const auto points = pool.polygonPoints(handle, index);
    for (const auto& rect : idb::refactor::decomposePolygonToRectangles(points)) {
      std::invoke(function, rect);
    }
  }
}

idb::refactor::Rect boundingBox(const std::vector<idb::refactor::Rect>& rects)
{
  idb::refactor::Rect box;
  bool first = true;
...
struct LayeredRect
{
  idb::refactor::TechLayerId layer;
  idb::refactor::Rect rect;
...
ViaShapes techViaShapes(const idb::refactor::TechDatabase& tech, idb::refactor::TechViaMasterId via_id, idb::refactor::Point origin)
{
  ViaShapes shapes;
  const auto& geometry = tech.viaMasterStorage().geometry(via_id);
  auto append = [&](idb::refactor::TechLayerId layer, const std::vector<idb::refactor::Rect>& rects, bool cut) {
    for (const auto& rect : rects) {
  ...
  append(idb::refactor::TechLayerId{geometry.cut_layer.entity()}, geometryRects(tech.geometryPool(), geometry.cut_geometry), true);
  append(geometry.top_layer.layer(), geometryRects(tech.geometryPool(), geometry.top_geometry), false);
  return shapes;
}

ViaShapes generatedViaShapes(const idb::refactor::DesignVia& via, idb::refactor::Point origin)
{
  ...
  const int32_t origin_x
      = (generated.flags & idb::refactor::DesignGeneratedViaFlag::kHasOrigin) != 0 ? generated.origin.x : 0;
  const int32_t origin_y
      = (generated.flags & idb::refactor::DesignGeneratedViaFlag::kHasOrigin) != 0 ? generated.origin.y : 0;
  ...
  auto append = [&](idb::refactor::TechLayerId layer, idb::refactor::Rect rect, bool cut) {
    LayeredRect placed{.layer = layer, .rect = rect.offset(origin.x, origin.y)};
    if (cut) {
      shapes.cut.push_back(placed);
    } else {
  ...
  for (int32_t row = 0; row < rows; ++row) {
    for (int32_t column = 0; column < columns; ++column) {
      ...
      append(idb::refactor::TechLayerId{generated.cut_layer.entity()},
             idb::refactor::Rect{.ll_x = ll_x, .ll_y = ll_y, .ur_x = ll_x + generated.cut_size_x, .ur_y = ll_y + generated.cut_size_y},
             true);
    }
  }

  const idb::refactor::Rect cut_box{.ll_x = ll_x_min,
                                    .ll_y = ll_y_min,
                                    .ur_x = ll_x_min + cut_width_total,
                                    .ur_y = ll_y_min + cut_height_total};
  append(idb::refactor::TechLayerId{generated.bottom_layer.entity()},
         idb::refactor::Rect{.ll_x = cut_box.ll_x - generated.bottom_enclosure_x,
         ...
         false);
  append(idb::refactor::TechLayerId{generated.top_layer.entity()},
         idb::refactor::Rect{.ll_x = cut_box.ll_x - generated.top_enclosure_x,
...
ViaShapes designViaShapes(const idb::refactor::DesignDatabase& design, const idb::refactor::TechDatabase& tech,
                          idb::refactor::DesignViaId via_id, idb::refactor::Point origin)
{
  const auto& via = design.routingStorage().via(via_id);
  if ((via.flags & idb::refactor::DesignViaFlag::kGenerated) != 0) {
    return generatedViaShapes(via, origin);
  }

  ViaShapes shapes;
  auto append_rect = [&](idb::refactor::TechLayerId layer, idb::refactor::Rect rect) {
    LayeredRect placed{.layer = layer, .rect = rect.offset(origin.x, origin.y)};
    if (tech.cutLayerStorage().contains(idb::refactor::TechCutLayerId{layer.entity()})) {
      shapes.cut.push_back(placed);
    } else {
      shapes.routing.push_back(placed);
    }
  ...
  for (const auto& polygon : via.polygons) {
    for (const auto& rect : idb::refactor::decomposePolygonToRectangles(polygon.points)) {
...
ViaShapes resolveViaShapes(const idb::refactor::DesignDatabase& design, const idb::refactor::TechDatabase& tech,
                           idb::refactor::TechViaMasterId tech_via, idb::refactor::DesignViaId design_via, idb::refactor::Point origin)
{
  if (tech_via) {
...
int32_t effectiveMinWidth(const idb::refactor::TechRoutingLayer& tech_layer)
{
  // iDB lef_read defaults omitted MINWIDTH to WIDTH.
  return (tech_layer.flags & idb::refactor::TechRoutingLayerFlag::kHasMinWidth) != 0 ? tech_layer.min_width : tech_layer.width;
}

void wrapTrackAxisFromEnTT(RoutingLayer& routing_layer, const idb::refactor::TechRoutingLayer& tech_layer)
{
  ...
  const int32_t offset_x = tech_layer.offset_form == idb::refactor::TechRoutingAxisValueForm::kNone ? tech_layer.pitch_x / 2
                                                                                                   : tech_layer.offset_x;
  const int32_t offset_y = tech_layer.offset_form == idb::refactor::TechRoutingAxisValueForm::kNone ? tech_layer.pitch_y / 2
                                                                                                   : tech_layer.offset_y;
...
void wrapRoutingDesignRuleFromEnTT(RoutingLayer& routing_layer, idb::refactor::TechRoutingLayerId layer_id,
                                   const idb::refactor::TechDatabase& tech)
{
  const auto& storage = tech.routingLayerStorage();
  const auto& tech_layer = storage.routingLayer(layer_id);
  const auto& name = tech.layerInfo(idb::refactor::TechLayerId{layer_id.entity()}).name;
  ...
  if (!prl_ids.empty()) {
    ...
    if (!spacing_ids.empty()) {
      ...
      for (const auto spacing_id : spacing_ids) {
        ...
        rows.push_back(SpacingRow{.is_default = src.type == idb::refactor::TechRoutingSpacingType::kDefault,
  ...
  if (!eol_ids.empty()) {
    ...
    routing_layer.set_eol_ete((src.flags & idb::refactor::TechRoutingLef58SpacingEolRuleFlag::kHasEndToEnd) != 0 ? src.end_to_end_space
                                                                                                                 : 0);
...
void wrapCutDesignRuleFromEnTT(CutLayer& cut_layer, idb::refactor::TechCutLayerId layer_id, const idb::refactor::TechDatabase& tech)
{
  ...
  if (!spacing_ids.empty()) {
    ...
    const idb::refactor::TechCutLef58SpacingTableRule* same_table = nullptr;
    for (const auto table_id : table_ids) {
      const auto& table = storage.lef58SpacingTableRule(table_id);
      if ((table.flags & idb::refactor::TechCutLef58SpacingTableRuleFlag::kHasSecondLayer) == 0) {
        same_table = &table;
      }
    }
  ...
  const idb::refactor::TechCutLef58SpacingTableRule* below_table = nullptr;
  for (const auto table_id : table_ids) {
    const auto& table = storage.lef58SpacingTableRule(table_id);
    if ((table.flags & idb::refactor::TechCutLef58SpacingTableRuleFlag::kHasSecondLayer) != 0) {
      below_table = &table;
...
idb::refactor::Rect wireSegmentBox(idb::refactor::Point a, idb::refactor::Point b, int32_t width)
{
  if (a.y == b.y) {
    return idb::refactor::Rect{.ll_x = std::min(a.x, b.x),
                               .ll_y = a.y - width / 2,
                               .ur_x = std::max(a.x, b.x),
                               .ur_y = a.y - width / 2 + width};
  }
  return idb::refactor::Rect{.ll_x = a.x - width / 2,
...
void appendObstacle(ObstacleSink& sink, const EnttLayerIndex& index, idb::refactor::Rect rect)
{
  if (index.wrap_idx < 0) {
    return;
  }
...
void appendObstacle(ObstacleSink& sink, idb::refactor::TechLayerId layer, idb::refactor::Rect rect)
{
...
struct NetSkipCache
{
  ...
  [[nodiscard]] const NetSkipInfo* find(idb::refactor::DesignNetId net) const noexcept
  {
...
NetSkipInfo analyzeNetSkip(const idb::refactor::DesignDatabase& design, const WrapperEntityTables& entities, idb::refactor::DesignNetId net)
{
  NetSkipInfo info;
  if (!net) {
    info.skip = true;
  ...
  const auto* io_pin_component = design_registry.try_get<const idb::refactor::DesignNetIoPins>(net.entity());
  const auto* instance_pin_component = design_registry.try_get<const idb::refactor::DesignNetInstancePins>(net.entity());
  const auto io_pins = io_pin_component == nullptr ? std::span<const idb::refactor::DesignIoPinId>{}
                                                    : std::span<const idb::refactor::DesignIoPinId>{io_pin_component->values};
  const auto instance_pins = instance_pin_component == nullptr ? std::span<const idb::refactor::DesignInstancePinId>{}
                                                                : std::span<const idb::refactor::DesignInstancePinId>{instance_pin_component->values};
...
bool netIsSkipping(const idb::refactor::DesignDatabase& design, const WrapperEntityTables& entities, idb::refactor::DesignNetId net, NetSkipCache& cache,
                   bool with_log)
{
  if (!net) {
    return true;
...
idb::refactor::DesignNetId instancePinNet(const idb::refactor::DesignInstancePin& pin)
{
  return pin.net;
}

idb::refactor::DesignNetId ioPinNet(const idb::refactor::DesignIoPin& pin)
{
  return pin.net;
}
...
void appendOffsetViaToPin(Pin& pin, const ViaShapes& shapes, idb::refactor::Point offset, const EnttLayerTable& layers,
...
class OrientedGeometryCache
{
 public:
  ...
  const Geometry& get(Id id, idb::refactor::DesignOrientation orientation, Builder&& builder)
  {
  ...
  [[nodiscard]] const Geometry& at(Id id, idb::refactor::DesignOrientation orientation) const
  {
...
class FixedObstacleSink
{
 public:
  ...
  void append(const CachedMasterGeometry& geometry, idb::refactor::Point origin)
  {
...
{
  idb::refactor::LibraryCellMasterId master;
  idb::refactor::Point origin;
  idb::refactor::DesignOrientation orientation = idb::refactor::DesignOrientation::kN;
  std::span<const idb::refactor::DesignInstancePinId> pins;
...
void RTInterface::wrapDatabaseFromEnTT()
{
  ...
  const auto net_instance_pins_for = [&design_registry](idb::refactor::DesignNetId net) {
    const auto* pins = design_registry.try_get<const idb::refactor::DesignNetInstancePins>(net.entity());
    return pins == nullptr ? std::span<const idb::refactor::DesignInstancePinId>{}
                           : std::span<const idb::refactor::DesignInstancePinId>{pins->values};
  };
  const auto net_io_pins_for = [&design_registry](idb::refactor::DesignNetId net) {
    const auto* pins = design_registry.try_get<const idb::refactor::DesignNetIoPins>(net.entity());
    return pins == nullptr ? std::span<const idb::refactor::DesignIoPinId>{}
                           : std::span<const idb::refactor::DesignIoPinId>{pins->values};
  };
  ...
  const auto build_obs_geometry = [&](idb::refactor::LibraryCellMasterId master_id, idb::refactor::DesignOrientation orientation) {
    ...
    if (obs != nullptr) {
      for (const auto& clause : obs->layer_clauses) {
        ...
        forEachGeometryRect(library.geometryPool(), clause.geometry, [&](const idb::refactor::Rect& rect) {
  ...
  const auto build_term_geometry = [&](idb::refactor::LibraryMasterTermId term_id, idb::refactor::DesignOrientation orientation) {
    CachedMasterGeometry geometry;
  ...
  for (const auto layer : tech.layerSequence()) {
    ...
    if (index->routing) {
      const auto routing_id = idb::refactor::TechRoutingLayerId{layer.entity()};
      ...
      const auto cut_id = idb::refactor::TechCutLayerId{layer.entity()};
      CutLayer cut_layer;
...

  {
    ...
    for (const auto via_id : tech.viaMasterStorage().viaMasters()) {
      ...
      if ((master.flags & idb::refactor::TechViaMasterFlag::kDefault) == 0) {
        continue;
      }
      const auto& geometry = tech.viaMasterStorage().geometry(via_id);
      ...
      const auto* cut_index = layers.find(idb::refactor::TechLayerId{geometry.cut_layer.entity()});
      ...
      if (geometry.top_layer.kind == idb::refactor::TechConductorLayerKind::kRouting) {
        via_master.set_above_direction(directionFromTech(
            tech.routingLayerStorage().routingLayer(idb::refactor::TechRoutingLayerId{geometry.top_layer.entity}).direction));
      }
      ...
      if (geometry.bottom_layer.kind == idb::refactor::TechConductorLayerKind::kRouting) {
        via_master.set_below_direction(directionFromTech(
            tech.routingLayerStorage().routingLayer(idb::refactor::TechRoutingLayerId{geometry.bottom_layer.entity}).direction));
      }
  ...
  }
  {
    ...
    design.netlistStorage().forEachSpecialNet([&](const auto net_id, const auto&) {
      design.routingStorage().forEachWire(net_id, [&](const auto wire_id, const auto&) {
        ...
        for (std::size_t path_index = 0; path_index < path_count; ++path_index) {
          ...
          const auto* path_layer = layers.find(idb::refactor::TechLayerId{path.layer().entity()});
          if (path.vias().empty()) {
            ...
            idb::refactor::Point first;
            idb::refactor::Point second;
            ...
            const auto rectangle = ordinary_point_count == 2 ? wireSegmentBox(first, second, path.width()) : idb::refactor::Rect{};
          ...
          for (const auto via : path.vias()) {
            ...
            for (uint32_t row = 0; row < rows; ++row) {
              for (uint32_t column = 0; column < columns; ++column) {
                const idb::refactor::Point origin{base.x + static_cast<int32_t>(column) * via.step_x,
      ...
      if (const auto* geometry = design.routingStorage().netGeometry(net_id); geometry != nullptr) {
        for (const auto& rect : geometry->rectangles) {
          appendObstacle(sink, idb::refactor::TechLayerId{rect.layer.entity()}, rect.rectangle);
        }
        for (const auto& polygon : geometry->polygons) {
          for (const auto& rect : idb::refactor::decomposePolygonToRectangles(polygon.points)) {
            appendObstacle(sink, idb::refactor::TechLayerId{polygon.layer.entity()}, rect);
          }
        }
        for (const auto& via : geometry->vias) {
    ...
    design.netlistStorage().forEachIoPin([&](const auto, const auto& io_pin) {
      ...
      for (const auto& port : io_pin.ports) {
        if (port.placement_status == idb::refactor::DesignPlacementStatus::kUnplaced
            && (port.flags & idb::refactor::DesignIoPinPortFlag::kHasPlacement) == 0) {
          continue;
        }
...
#pragma omp parallel for schedule(static)
    ...
    for (const auto& work : instance_work) {
      ...
      const bool is_block = master.type == idb::refactor::LibraryCellMasterType::kBlock
                            || master.type == idb::refactor::LibraryCellMasterType::kBlockBlackbox
                            || master.type == idb::refactor::LibraryCellMasterType::kBlockSoft;
      if (!is_block || active_routing_layer_indices.empty()) {
        continue;
      }

      const auto body = placeMasterRect(idb::refactor::Rect{.ll_x = 0,
                                                            .ll_y = 0,
      ...
      std::vector<std::vector<idb::refactor::Rect>> obstacles_by_layer(static_cast<size_t>(layers.routingCount()));
      ...
      for (const auto& shape : work.obs_geometry->routing) {
        ...
        const idb::refactor::Rect overlap{.ll_x = std::max(body.ll_x, placed.ll_x),
      ...
      for (size_t x = 0; x + 1 < x_coordinates.size(); ++x) {
        for (size_t y = 0; y + 1 < y_coordinates.size(); ++y) {
          const idb::refactor::Rect tile{.ll_x = x_coordinates[x],
          ...
          for (const int32_t layer_idx : active_routing_layer_indices) {
            ...
            const bool blocked = std::ranges::any_of(
                layer_obstacles, [&](const idb::refactor::Rect obstacle) { return tile.intersects(obstacle, false); });
            if (!blocked) {
...

  {
    ...
    std::vector<idb::refactor::DesignNetId> valid_nets;
    design.netlistStorage().forEachRegularNet([&](const auto net_id, const auto&) {
...
#pragma omp for schedule(guided, 256)
      for (size_t index = 0; index < valid_nets.size(); ++index) {
        ...
        for (const auto pin_id : instance_pins) {
          const auto& instance_pin = entities.instance_pins.get(pin_id.entity());
          const auto& term = entities.master_terms.get(instance_pin.master_term.entity());
          if (term.direction == idb::refactor::LibraryMasterTermDirection::kOutput
              || term.direction == idb::refactor::LibraryMasterTermDirection::kOutputTriState) {
        ...
        if (!found_driven) {
          for (const auto pin_id : io_pins) {
            const auto& io_pin = entities.io_pins.get(pin_id.entity());
            if (io_pin.direction == idb::refactor::DesignIoPinDirection::kInput) {
        ...
        if (!found_driven) {
          for (const auto pin_id : io_pins) {
            const auto& io_pin = entities.io_pins.get(pin_id.entity());
            if (io_pin.direction == idb::refactor::DesignIoPinDirection::kInOut) {
              driven_name = io_pin.name;
              found_driven = true;
              break;
            }
...
idb::refactor::TechRoutingLayerId requireEnttRoutingLayer(const idb::refactor::TechDatabase& tech,
                                                          std::vector<RoutingLayer>& routing_layers, int32_t layer_idx)
{
  ...
  const idb::refactor::TechRoutingLayerId routing_layer{layer.entity()};
...
void RTInterface::outputTrackGridToEnTT()
{
  ...
  std::vector<idb::refactor::DesignTrackGrid> replacements;
  for (int32_t index = static_cast<int32_t>(routing_layers.size()) - 1; index >= 0; --index) {
    ...
    for (const ScaleGrid& grid : axis.get_x_grid_list()) {
      replacements.push_back(idb::refactor::DesignTrackGrid{.axis = idb::refactor::DesignAxis::kX,
    ...
    for (const ScaleGrid& grid : axis.get_y_grid_list()) {
      replacements.push_back(idb::refactor::DesignTrackGrid{.axis = idb::refactor::DesignAxis::kY,
                                                            .start = grid.get_start_line(),
                                                            .track_count = static_cast<uint32_t>(grid.get_step_num() + 1),
                                                            .step = grid.get_step_length(),
                                                            .layers = {layer}});
  ...
  std::vector<idb::refactor::DesignTrackGridId> created;
  try {
...
void RTInterface::outputGCellGridToEnTT()
{
  ...
  std::vector<idb::refactor::DesignGCellGrid> replacements;
  for (const ScaleGrid& grid : axis.get_x_grid_list()) {
    replacements.push_back(idb::refactor::DesignGCellGrid{.axis = idb::refactor::DesignAxis::kX,
                                                          .start = grid.get_start_line(),
                                                          .line_count = static_cast<uint32_t>(grid.get_step_num() + 1),
                                                          .step = grid.get_step_length()});
  }
  for (const ScaleGrid& grid : axis.get_y_grid_list()) {
    replacements.push_back(idb::refactor::DesignGCellGrid{.axis = idb::refactor::DesignAxis::kY,
                                                          .start = grid.get_start_line(),
                                                          .line_count = static_cast<uint32_t>(grid.get_step_num() + 1),
                                                          .step = grid.get_step_length()});
  }

  std::vector<idb::refactor::DesignGCellGridId> created;
  try {
    created.reserve(replacements.size());
    for (auto& grid : replacements) {
...
void RTInterface::outputNetListToEnTT()
{
  struct PendingWire
  {
    idb::refactor::DesignNetId net;
    idb::refactor::DesignWireRoutingInput routing;
  };
  ...
  for (const int32_t net_idx : net_order) {
    ...
    idb::refactor::DesignWireRoutingInput input;
    if (const auto found = result_map.find(net_idx); found != result_map.end()) {
      ...
      for (const auto* segment : segments) {
        ...
        idb::refactor::DesignWirePath path;
        if (first.get_layer_idx() == second.get_layer_idx()) {
          ...
          path.vias.push_back(idb::refactor::DesignWireVia{.point_index = 0, .tech_via = tech_via, .design_via = design_via});
        }
    ...
    if (const auto found = patch_map.find(net_idx); found != patch_map.end()) {
      ...
      for (const auto* patch : patches) {
        ...
        path.rectangles.push_back(idb::refactor::DesignWireRectangle{
            .point_index = 0,
            .delta = {.ll_x = 0,
                      .ll_y = 0,
  ...
  std::vector<idb::refactor::DesignWireId> old_wires;
  ...
  std::vector<idb::refactor::DesignWireId> created;
  try {
    created.reserve(pending.size());
    for (auto& wire : pending) {
      created.push_back(routing.createWire(idb::refactor::DesignWire{.net = wire.net, .status = idb::refactor::DesignWireStatus::kRouted},
  ...
  for (size_t i = 0; i < left_nets.size(); ++i) {
    ...
    for (size_t pin_idx = 0; pin_idx < left_pins.size(); ++pin_idx) {
      ...
      if (left_pin.get_pin_name() != right_pin.get_pin_name() || left_pin.get_is_core() != right_pin.get_is_core()
          || left_pin.get_is_driven() != right_pin.get_is_driven()) {
