...
int32_t routingThreadNumber()
{
  ...
  const char* value = std::getenv("IDB_REFACTOR_RT_THREAD_NUMBER");
  ...
  if (consumed != text.size() || parsed <= 0) {
    throw std::invalid_argument("IDB_REFACTOR_RT_THREAD_NUMBER must be a positive integer");
  }
  return parsed;
}
...
{
  return envPath("IDB_REFACTOR_ISPD18_ROOT");
}

std::filesystem::path ispd19Source()
{
  return envPath("IDB_REFACTOR_ISPD19_ROOT");
}
...
class TemporaryWorkspace
{
 public:
  ...
  ~TemporaryWorkspace()
  {
    if (std::getenv("IDB_REFACTOR_KEEP_TEMP") != nullptr) {
...
class EnttDesignFixture
{
 public:
      ...
      : library(technology.techRegistry()), design(technology.techRegistry(), library.libraryRegistry())
  {
    ...
    idb::refactor::LefTechImporter(technology).import(lef);
    idb::refactor::LefLibraryImporter(technology, library).import(lef);
    idb::refactor::DefDesignImporter(design).import(def);
  }

  idb::refactor::TechDatabase technology;
  idb::refactor::LibraryDatabase library;
  idb::refactor::DesignDatabase design;
};
...
void runInjectedWriteback(const std::filesystem::path& temp_directory, idb::refactor::DesignDatabase* design = nullptr,
                          idb::refactor::TechDatabase* technology = nullptr, idb::refactor::LibraryDatabase* library = nullptr)
{
  Logger::initInst();
...
void expectWritebackSemanticsEqual(const idb::refactor::DesignDatabase& expected, const idb::refactor::DesignDatabase& actual,
                                   bool compare_vias = true)
{
  const auto expected_snapshot = idb::refactor::test::makeDesignSemanticSnapshot(expected);
  const auto actual_snapshot = idb::refactor::test::makeDesignSemanticSnapshot(actual);
  ...
  idb::refactor::test::expectStructuredNetSemantics(expected, actual);
}

using DrcShapeKey = std::tuple<int32_t, int32_t, bool, int32_t, int32_t, int32_t, int32_t>;

std::vector<DrcShapeKey> canonicalDrcShapes(const std::vector<ids::Shape>& shapes)
{
  std::vector<DrcShapeKey> result;
  result.reserve(shapes.size());
  for (const auto& shape : shapes) {
    result.emplace_back(shape.net_idx, shape.layer_idx, shape.is_routing, shape.ll_x, shape.ll_y, shape.ur_x, shape.ur_y);
  }
  std::sort(result.begin(), result.end());
  return result;
}

std::vector<idb::refactor::test::structured::CanonicalNet> canonicalSpecialNets(const idb::refactor::DesignDatabase& design)
{
  std::vector<idb::refactor::test::structured::CanonicalNet> result;
  for (const auto net : design.netlistStorage().specialNets()) {
    result.push_back(idb::refactor::test::structured::canonicalNet(design, net));
  }
...
{
  idb::refactor::TechDatabase tech;
  std::unique_ptr<idb::refactor::LibraryDatabase> library;
  std::unique_ptr<idb::refactor::DesignDatabase> design;

  // SI2 LEF/DEF parsers are process-global. Clear leftover callback and
  // parser state before starting the EnTT import sequence.
  static_cast<void>(LefDefParser::lefrClear());
  static_cast<void>(LefDefParser::defrClear());
  idb::refactor::LefTechImporter(tech).import(lef);
  library = std::make_unique<idb::refactor::LibraryDatabase>(tech.techRegistry());
  idb::refactor::LefLibraryImporter(tech, *library).import(lef);
  design = std::make_unique<idb::refactor::DesignDatabase>(tech.techRegistry(), library->libraryRegistry());
  idb::refactor::DefDesignImporter(*design).import(def);
  loadIntoIdb(lef, def);
                                  ...
                                  const std::filesystem::path& temp_directory)
{
  ...
  static_cast<void>(LefDefParser::defrClear());
  idb::refactor::LefTechImporter(tech).import(lef);
  auto library = std::make_unique<idb::refactor::LibraryDatabase>(tech.techRegistry());
  idb::refactor::LefLibraryImporter(tech, *library).import(lef);
  auto design = std::make_unique<idb::refactor::DesignDatabase>(tech.techRegistry(), library->libraryRegistry());
  idb::refactor::DefDesignImporter(*design).import(def);
...
TEST_P(IspdRoutedWritebackDifferentialTest, NativeEnttMatchesLegacyIdbAfterRouting)
{
  ...
  if (root.empty()) {
    GTEST_SKIP() << "set " << (test_case.corpus == IspdCorpus::k2018 ? "IDB_REFACTOR_ISPD18_ROOT" : "IDB_REFACTOR_ISPD19_ROOT")
  ...
  const pid_t entt_pid = launchRoutingWorker(entt_log, [&] {
    ...
    idb::refactor::DefDesignExporter(fixture.design).write(entt_output_def);
  ...
  const auto idb_shapes = canonicalDrcShapes(DRCI.buildResultShapeList());
  DRCI.setDesignSource(&actual.design, &actual.technology, &actual.library);
  const auto entt_shapes = canonicalDrcShapes(DRCI.buildResultShapeList());
...
TEST(SelfCheck, TwoIdbWrapsMatch)
{
  ...
  if (root.empty()) {
    GTEST_SKIP() << "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus";
  }
...
TEST(Writeback, NativeEnttMatchesLegacyIdbDefAndRoundTrips)
{
  ...
  const auto special_before = idb::refactor::test::structured::canonicalNet(actual.design, special_id);
  ...
  EXPECT_EQ(special_before, idb::refactor::test::structured::canonicalNet(actual.design, special_id));

  idb::refactor::DefDesignExporter(actual.design).write(entt_output_def);
  idb::refactor::DesignDatabase roundtripped(actual.technology.techRegistry(), actual.library.libraryRegistry());
  idb::refactor::DefDesignImporter(roundtripped).import(entt_output_def);
  expectWritebackSemanticsEqual(actual.design, roundtripped);

  const auto first_export = idb::refactor::DefDesignExporter(actual.design).exportText();
  ...
  EXPECT_EQ(idb::refactor::DefDesignExporter(actual.design).exportText(), first_export);
  EXPECT_EQ(special_before, idb::refactor::test::structured::canonicalNet(actual.design, special_id));
  expectWritebackSemanticsEqual(expected.design, actual.design);
...
TEST(Diagnostic, DISABLED_Ispd19Test2BuildOrder)
{
  ...
  if (root.empty()) {
    GTEST_SKIP() << "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus";
  }
...
TEST(RunSnapshot, Ispd18Sample3IdbOnce)
{
  ...
  const auto output = envPath("IDB_REFACTOR_RT_SNAPSHOT_OUT");
  if (output.empty()) {
    GTEST_FAIL() << "set IDB_REFACTOR_RT_SNAPSHOT_OUT to the snapshot output path";
  }
...
{
  const auto lef = envPath("IDB_REFACTOR_IRT_WRAP_LEF");
  const auto def = envPath("IDB_REFACTOR_IRT_WRAP_DEF");
  if (lef.empty() || def.empty()) {
    GTEST_SKIP() << "set IDB_REFACTOR_IRT_WRAP_LEF and IDB_REFACTOR_IRT_WRAP_DEF";
  }
...
TEST(Wrap, Ispd18Sample) { compareIdbAndEntt({"ispd18_sample"}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Sample2) { compareIdbAndEntt({"ispd18_sample2"}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Sample3) { compareIdbAndEntt({"ispd18_sample3"}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test1) { compareIdbAndEntt({"ispd18_test1"}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test2) { compareIdbAndEntt({"ispd18_test2"}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test3) { compareIdbAndEntt({"ispd18_test3"}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test4) { compareIdbAndEntt({"ispd18_test4", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test5) { compareIdbAndEntt({"ispd18_test5", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test6) { compareIdbAndEntt({"ispd18_test6", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test7) { compareIdbAndEntt({"ispd18_test7", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test8) { compareIdbAndEntt({"ispd18_test8", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test9) { compareIdbAndEntt({"ispd18_test9", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd18Test10) { compareIdbAndEntt({"ispd18_test10", true}, ispd18Source(), "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD18 corpus"); }
TEST(Wrap, Ispd19Sample)
{
  compareIdbAndEntt({"ispd19_sample"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Sample2)
{
  compareIdbAndEntt({"ispd19_sample2"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Sample3)
{
  compareIdbAndEntt({"ispd19_sample3"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Sample4)
{
  compareIdbAndEntt({"ispd19_sample4"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test1)
{
  compareIdbAndEntt({"ispd19_test1"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test2)
{
  compareIdbAndEntt({"ispd19_test2"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test3)
{
  compareIdbAndEntt({"ispd19_test3"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test4)
{
  compareIdbAndEntt({"ispd19_test4"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test5)
{
  compareIdbAndEntt({"ispd19_test5"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test6)
{
  compareIdbAndEntt({"ispd19_test6"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test7)
{
  compareIdbAndEntt({"ispd19_test7"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test8)
{
  compareIdbAndEntt({"ispd19_test8"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test9)
{
  compareIdbAndEntt({"ispd19_test9"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
TEST(Wrap, Ispd19Test10)
{
  compareIdbAndEntt({"ispd19_test10"}, ispd19Source(), "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD19 corpus");
}
