...
#include "integration/test/DesignSemanticSnapshot.h"
...
struct DesignFixture
{
  ...
  DesignDatabase design{technology.techRegistry(), library.libraryRegistry()};
...
TEST(DesignBinaryDatabaseArchiveTest, LargeIspd19RoutingPoolReachesByteExactFixedPoint)
{
  ...
  if (std::getenv("IDB_REFACTOR_RUN_LARGE_DESIGN_BINARY_TESTS") == nullptr) {
    GTEST_SKIP() << "set IDB_REFACTOR_RUN_LARGE_DESIGN_BINARY_TESTS=1 to run the large Design binary archive test";
  }
  const char* root_value = std::getenv("IDB_REFACTOR_ISPD19_ROOT");
  ...
  auto source = std::make_unique<DesignDatabase>(technology.techRegistry(), library.libraryRegistry());
