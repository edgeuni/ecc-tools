...
{
  const auto directory = std::filesystem::path(testing::TempDir()) / "idb_refactor_def_corpus";
...
{
  if (std::getenv("IDB_REFACTOR_RUN_LARGE_DEF_TESTS") == nullptr) {
    GTEST_SKIP() << "set IDB_REFACTOR_RUN_LARGE_DEF_TESTS=1 to run the large DEF stress test";
  }
  const auto root = requireOpenRoadSource();
  if (root.empty()) {
    GTEST_SKIP() << "set OPENROAD_SOURCE_DIR to an OpenROAD source checkout";
