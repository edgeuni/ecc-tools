...
{
  if (argc != 3) {
    std::cerr << "usage: idb_refactor_entt_memory_probe <lef> <def>\n";
    return 2;
  }
  ...
  try {
    ...
    const auto baseline = idb::refactor::settledMemory();
    ...
    idb::refactor::LibraryDatabase library(technology.techRegistry());
    idb::refactor::LefLibraryImporter(technology, library).import(lef);
    const auto lef_milliseconds = idb::refactor::elapsedMilliseconds(lef_start);
    const auto after_lef = idb::refactor::settledMemory();

    const auto def_start = std::chrono::steady_clock::now();
    idb::refactor::DesignDatabase design(technology.techRegistry(), library.libraryRegistry());
    idb::refactor::DefDesignImporter importer(design);
    importer.import(def);
    const auto def_milliseconds = idb::refactor::elapsedMilliseconds(def_start);
    const auto after_def = idb::refactor::settledMemory();
    const auto counts = idb::refactor::countDatabase(technology, library, design);
    const auto breakdown = idb::refactor::memoryBreakdown(design, after_def.allocator_in_use_kib * 1024u);

    std::cout << "ENTT_MEMORY_JSON={\"baseline\":";
    idb::refactor::printMemory(baseline);
    std::cout << ",\"after_lef\":";
    idb::refactor::printMemory(after_lef);
    std::cout << ",\"after_def\":";
    idb::refactor::printMemory(after_def);
    ...
    idb::refactor::printCounts(counts);
    std::cout << ",\"memory_breakdown\":";
    idb::refactor::printMemoryBreakdown(breakdown);
    std::cout << "}\n";
    return 0;
