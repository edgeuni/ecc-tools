...
inline std::vector<std::string> ioPinKeys(const DesignDatabase& design)
{
  std::vector<std::string> result;
  for (const auto id : design.netlistStorage().ioPins()) {
    ...
    key.unsignedValue(static_cast<uint8_t>(pin.use));
...
inline void appendWirePathSemantics(SemanticKey& key, const DesignDatabase& design, const DesignWire& wire, DesignWirePathView path)
{
  ...
  key.unsignedValue(path.flags());
  key.signedValue(path.width());
  key.unsignedValue(path.mask());
...
inline std::vector<std::string> wirePrimitiveKeys(const DesignDatabase& design, DesignNetId id)
{
  std::vector<std::string> result;
  for (const auto wire_id : design.routingStorage().wires(id)) {
    ...
    for (std::size_t path_index = 0; path_index < design.routingStorage().pathCount(wire_id); ++path_index) {
      ...
      for (std::size_t point_index = 1; point_index < path.points().size(); ++point_index) {
        ...
        SemanticKey key;
        key.text("segment");
        appendWirePathSemantics(key, design, wire, path);
...
inline DesignSemanticSnapshot makeDesignSemanticSnapshot(const DesignDatabase& design, bool include_net_keys = true)
{
  return DesignSemanticSnapshot{
      ...
      .io_pins = detail::ioPinKeys(design),
