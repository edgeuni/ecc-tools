...
{
  if (const char* value = std::getenv("IDB_REFACTOR_ISPD18_ROOT"); value != nullptr && value[0] != '\0') {
    return value;
  }
  return {};
}
...
{
  if (const char* value = std::getenv("IDB_REFACTOR_ISPD19_ROOT"); value != nullptr && value[0] != '\0') {
    return value;
  }
  return {};
}
...
void expectRepresentedSemantics(const OpenDbCorpusCase& test_case, const DesignDatabase& expected, const DesignDatabase& actual)
{
  ...
  if (test_case.large) {
    expectSemanticSnapshot(test::makeDesignSemanticSnapshot(expected, false), test::makeDesignSemanticSnapshot(actual, false));
    test::expectStructuredNetSemantics(expected, actual);
  } else {
    expectSemanticSnapshot(test::makeDesignSemanticSnapshot(expected), test::makeDesignSemanticSnapshot(actual));
    expectAllNetRoutingSemantics(expected, actual);
  }
}
...
TEST_P(OpenDbCorpusDifferentialTest, MatchesOpenDbNormalizedImportAndExportSemantics)
{
  ...
  if (test_case.large && test_case.root == OpenDbCorpusRoot::kOpenRoad && std::getenv("IDB_REFACTOR_RUN_LARGE_DEF_TESTS") == nullptr) {
    GTEST_SKIP() << "set IDB_REFACTOR_RUN_LARGE_DEF_TESTS=1 to run the large OpenDB differential test";
  }
  ...
  if (root.empty() || !std::filesystem::is_directory(root)) {
    if (test_case.root == OpenDbCorpusRoot::kIspd18) {
      GTEST_SKIP() << "set IDB_REFACTOR_ISPD18_ROOT to the extracted ISPD 2018 benchmark root";
    }
    if (test_case.root == OpenDbCorpusRoot::kIspd19) {
      GTEST_SKIP() << "set IDB_REFACTOR_ISPD19_ROOT to the extracted ISPD 2019 benchmark root";
    }
    GTEST_SKIP() << "set OPENROAD_SOURCE_DIR to an OpenROAD source checkout";
  }
  ...
  ASSERT_NO_THROW(source_importer.import(input_def)) << input_def;

  const auto exit_code = normalizeWithOpenDb(lefs, input_def, open_db_def, open_db_log);
  ASSERT_EQ(exit_code, 0) << readText(open_db_log);
  ASSERT_TRUE(std::filesystem::is_regular_file(open_db_def)) << readText(open_db_log);
