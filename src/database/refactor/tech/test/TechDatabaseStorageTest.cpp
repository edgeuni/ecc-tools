...
static_assert(std::is_same_v<decltype(std::declval<const TechDatabase&>().techRootId()), TechRootId>);
static_assert(std::is_same_v<decltype(std::declval<const TechDatabase&>().globalStorage()), const TechGlobalStorage&>);
...
class TechDatabaseStorageTest : public testing::Test
{
 protected:
  void SetUp() override
  {
...
TEST(TechDatabaseOptionsTest, RectangularizesManhattanViaPolygons)
{
  const TechDatabaseOptions options{.geometry = GeometryPoolOptions{.polygon_mode = PolygonStorageMode::kRectangularized}};
  TechDatabase database(options);
...
TEST_F(TechDatabaseStorageTest, CreatesOneTechRootInTheSharedRegistry)
{
...
TEST_F(TechDatabaseStorageTest, StoresLayerPropertiesOnTheLayerEntity)
{
...
TEST_F(TechDatabaseStorageTest, KeepsViaGeometryAndCandidatesInsideComponents)
{
...
TEST_F(TechDatabaseStorageTest, StoresNdrClausesAsTypedComponentsOnOneEntity)
{
  auto& vias = database.viaMasterStorage();
  const auto via
      = vias.createFixedViaMaster(TechViaMaster{.name = "VIA12"},
...
TEST_F(TechDatabaseStorageTest, DestroysOnlyNdrOwnedViaEntitiesWithTheirOwner)
{
  auto& vias = database.viaMasterStorage();
  const auto via
      = vias.createFixedViaMaster(TechViaMaster{.name = "VIA12"},
...
TEST_F(TechDatabaseStorageTest, StoresGlobalsAsComponentsOnTheTechRoot)
{
  auto& registry = database.techRegistry().registry();
...
TEST_F(TechDatabaseStorageTest, RemovesGlobalComponentsAndRepresentsAbsenceByMissingComponents)
{
  auto& registry = database.techRegistry().registry();
  auto& globals = database.globalStorage();
  const auto root = database.techRootId();
...
TEST_F(TechDatabaseStorageTest, ValidatesMaxViaStackRangesAgainstCurrentRoutingLayers)
{
  auto& registry = database.techRegistry().registry();
  auto& globals = database.globalStorage();
