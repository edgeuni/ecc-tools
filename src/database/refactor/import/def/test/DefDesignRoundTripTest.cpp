...
void expectComplexRouting(const DesignDatabase& design)
{
...
{
  const auto input = writeInput(kComplexDef, "idb_refactor_complex_input.def");
  DesignDatabase first{tech, library.libraryRegistry()};
  ...
  const auto output = std::filesystem::path(testing::TempDir()) / "idb_refactor_complex_output.def";
  exporter.write(output);
  DesignDatabase second{tech, library.libraryRegistry()};
...
{
  const auto input = writeInput(kComplexDef, "idb_refactor_failed_output_input.def");
...
END DESIGN
                                   ...
                                   "idb_refactor_recovered_after_rollback.def");
...
END DESIGN
                                ...
                                "idb_refactor_generated_via.def");
  DesignDatabase first{tech, library.libraryRegistry()};
  ...
  const auto output = writeInput(text, "idb_refactor_generated_via_output.def");
  DesignDatabase second{tech, library.libraryRegistry()};
...
END DESIGN
                                ...
                                "idb_refactor_shared_regular_special.def");
  ...
  const auto output = writeInput(text, "idb_refactor_shared_regular_special_output.def");
  DesignDatabase second{tech, library.libraryRegistry()};
  DefDesignImporter(second).import(output);
...
END DESIGN
                                ...
                                "idb_refactor_fill_mustjoin.def");
  ...
  const auto output = writeInput(text, "idb_refactor_fill_mustjoin_output.def");
  DesignDatabase second{tech, library.libraryRegistry()};
  DefDesignImporter reimporter(second);
  reimporter.import(output);
  EXPECT_TRUE(reimporter.diagnostics().empty());
...
END DESIGN
                                ...
                                "idb_refactor_forward_group_region.def");
...
END DESIGN
                                ...
                                "idb_refactor_unsupported_fills.def");
