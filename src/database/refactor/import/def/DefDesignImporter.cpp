...
{
  explicit DirectContext(DesignDatabase& database) : design(database) {}
...
void ensureEmptyTarget(const DesignDatabase& design)
{
  if (design.globalStorage().hasInfo() || design.globalStorage().hasDieArea() || design.floorplanStorage().hasCoreArea()
    ...
    throw std::logic_error("direct DEF importer requires an empty DesignDatabase");
  }
}
...
void rollbackImport(DesignDatabase& design) noexcept
{
  try {
...
void directResolvePending(DirectContext& context)
{
  ...
  for (const auto& pending : context.pending_connections) {
    ...
    if (isPinConnection(pending.instance) && !netlist.findIoPin(pending.pin)) {
      throw std::runtime_error("NET references an unknown top-level PIN: " + pending.pin);
    }
