...
```bash
cmake -S . -B build/refactor-benchmark \
  -DIDB_REFACTOR_BUILD_BENCHMARKS=ON \
  -DIDB_REFACTOR_BUILD_TESTS=OFF \
  -DBUILD_GUI=OFF -DBUILD_PYTHON=OFF
cmake --build build/refactor-benchmark --target idb_refactor_benchmark -j2
```

The resulting executable is `bin/idb_refactor_benchmark`.

The same configuration also builds
`bin/idb_refactor_binary_archive_benchmark`. It measures the current EnTTDB
...
```bash
cmake --build build/refactor-benchmark \
  --target idb_refactor_binary_archive_benchmark -j2

./bin/idb_refactor_binary_archive_benchmark \
  --lef reference/ispd2019/ispd19_test10/ispd19_test10.input.lef \
  --def reference/ispd2019/ispd19_test10/ispd19_test10.input.def \
  --archive-dir src/database/refactor/benchmark/results/ispd19-test10-input-binary \
  --output src/database/refactor/benchmark/results/ispd19-test10-input-binary.jsonl
```
