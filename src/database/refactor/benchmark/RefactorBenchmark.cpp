...
#if defined(IDB_REFACTOR_BENCHMARK_HAS_LEGACY_IDB)
#include "idm.h"
#endif
...
void benchmarkEntt(const Options& options, std::vector<Record>& records)
{
  ...
  const auto temp = std::filesystem::temp_directory_path() / ("idb-refactor-benchmark-" + std::to_string(getpid()));
...
void benchmarkLegacy(const Options& options, std::vector<Record>& records)
{
  ...
  const auto temp = std::filesystem::temp_directory_path() / ("idb-refactor-benchmark-idb-" + std::to_string(getpid()));
  std::filesystem::create_directories(temp);
...
Options parseOptions(int argc, char** argv)
{
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--lef") options.lef = value(argument);
      ...
      std::cout << "usage: idb_refactor_benchmark --lef FILE --def FILE [--source entt|idb|both] [--output FILE] [--writes N]\n";
      std::exit(0);
    } else {
...
{
  try {
    const auto options = idb::refactor::parseOptions(argc, argv);
    std::vector<idb::refactor::Record> records;
    if (options.source == "entt" || options.source == "both") idb::refactor::benchmarkEntt(options, records);
#if defined(IDB_REFACTOR_BENCHMARK_HAS_LEGACY_IDB)
    if (options.source == "idb" || options.source == "both") idb::refactor::benchmarkLegacy(options, records);
#else
    ...
    for (const auto& record : records) idb::refactor::writeJson(*destination, record, case_name);
    std::cerr << "benchmark completed: " << records.size() << " measurements, sink=" << idb::refactor::g_sink << '\n';
    return 0;
