...
struct EnttContext
{
  std::unique_ptr<idb::refactor::TechDatabase> tech;
...
{
  EnttContext context;
  context.tech = std::make_unique<idb::refactor::TechDatabase>();
  idb::refactor::LefTechImporter(*context.tech).import(options.lef);
  context.library = std::make_unique<idb::refactor::LibraryDatabase>(context.tech->techRegistry());
  idb::refactor::LefLibraryImporter(*context.tech, *context.library).import(options.lef);
  context.design = std::make_unique<idb::refactor::DesignDatabase>(context.tech->techRegistry(), context.library->libraryRegistry());
  idb::refactor::DefDesignImporter(*context.design).import(options.def);
  return context;
}
