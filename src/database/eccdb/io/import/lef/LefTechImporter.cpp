...
#include "import/lef/detail/LefTechLayerProperties.h"
#include "import/lef/detail/LefTechLayerRules.h"
...
void ensureEmptyTarget(const TechDatabase& database)
{
  ...
  if (!database.contains(database.techRootId()) || registry.storage<TechEntity>()->free_list() != 1u || !database.layerSequence().empty()
    ...
    throw std::logic_error("direct LEF importer requires a new empty TechDatabase");
  }
}

void rollbackImport(TechDatabase& database, GeometryPoolCheckpoint geometry_checkpoint) noexcept
{
...
void commitImport(TechDatabase& database, const PreparedImport& prepared)
{
