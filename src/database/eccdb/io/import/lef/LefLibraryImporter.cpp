...
struct Staging
{
  ...
  const TechDatabase* direct_technology = nullptr;
  LibraryDatabase* direct_library = nullptr;
...
void appendPreparedPath(std::vector<Rect>& target, const StagedPath& source, TechLayerId layer, const TechDatabase& technology,
                        int32_t units);
LibraryViaPlacement prepareVia(const StagedViaPlacement& source, const LefNamePool& names, const TechDatabase& technology, int32_t units);
...
LibraryMasterPortInput directPort(const lefiGeometries& geometry, const TechDatabase& technology,
                                  const Staging& staging, int32_t units);
LibraryMasterObsInput directObs(const lefiGeometries& geometry, const TechDatabase& technology,
                                const Staging& staging, int32_t units);

LibraryMasterPortInput materializePort(const StagedPort& source, const LefNamePool& names, const TechDatabase& technology, int32_t units)
{
...
int32_t directPathHalfWidth(double width, TechLayerId layer, const TechDatabase& technology, int32_t units)
{
  int32_t dbu_width = 0;
  if (width != 0.0) {
...
void appendDirectPath(std::vector<Rect>& target, const lefiGeomPath& path, double width, TechLayerId layer, const TechDatabase& technology,
...
LibraryMasterObsInput materializeObs(const StagedObs& source, const LefNamePool& names, const TechDatabase& technology, int32_t units)
{
  LibraryMasterObsInput result;
...
int32_t technologyUnits(const TechDatabase& technology)
{
...
int32_t pathWidth(const StagedPath& source, TechLayerId layer, const TechDatabase& technology, int32_t units)
{
  if (source.width > 0.0) {
...
std::vector<PreparedMaster> prepareMasters(const Staging& staging, const TechDatabase& technology, int32_t units)
{
...
void ensureEmptyTarget(const LibraryDatabase& library)
{
  const auto* entities = library.libraryRegistry().registry().storage<LibraryEntity>();
  if (entities != nullptr && entities->free_list() != 0u) {
    throw std::logic_error("direct LEF library importer requires a new empty LibraryDatabase");
  }
...
void rollbackImport(LibraryDatabase& library, GeometryPoolCheckpoint geometry_checkpoint) noexcept
{
...
void commitImport(LibraryDatabase& library, std::vector<LibrarySite> sites, std::vector<PreparedMaster> masters)
{
