...
std::unique_ptr<TechDatabase> BinaryDatabaseImporter::loadTech(const std::filesystem::path& path)
{
  std::unique_ptr<TechDatabase> result;
  binary_detail::readBinaryFile(
      ...
      [&result](std::istream& input, const binary_detail::BinaryFileHeader&) {
        ...
        result = std::make_unique<TechDatabase>(TechDatabaseOptions{.geometry = geometry.options});
...
std::unique_ptr<LibraryDatabase> BinaryDatabaseImporter::loadLibrary(const std::filesystem::path& path,
                                                                     const TechDatabase& technology)
{
  std::unique_ptr<LibraryDatabase> result;
  binary_detail::readBinaryFile(
      ...
      [&result, &technology](std::istream& input, const binary_detail::BinaryFileHeader&) {
        binary_detail::BinaryInputArchive archive(input);
        auto geometry = readGeometry(archive);
        result = std::make_unique<LibraryDatabase>(technology.techRegistry(), LibraryDatabaseOptions{.geometry = geometry.options});
        result->geometryPool().restoreSerialized(geometry.options, std::move(geometry.entries), std::move(geometry.rectangles),
                                                 std::move(geometry.points), std::move(geometry.polygons));
...
std::unique_ptr<DesignDatabase> BinaryDatabaseImporter::loadDesign(const std::filesystem::path& path, const TechDatabase& technology,
                                                                   const LibraryDatabase& library)
{
  std::unique_ptr<DesignDatabase> result;
  binary_detail::readBinaryFile(
      ...
      [&result, &technology, &library](std::istream& input, const binary_detail::BinaryFileHeader&) {
        ...
        result = std::make_unique<DesignDatabase>(technology.techRegistry(), library.libraryRegistry());
