...
{
 public:
  LibraryWriter(std::ostream& output, const TechDatabase& technology, const LibraryDatabase& library)
 ...
 private:
  ...
  std::ostream& _output;
  const TechDatabase& _technology;
  const LibraryDatabase& _library;
  int32_t _database_units_per_micron = 0;
};

}  // namespace

void LefLibraryExporter::write(std::ostream& output, const TechDatabase& technology, const LibraryDatabase& library)
{
  LibraryWriter(output, technology, library).write();
}

void LefLibraryExporter::write(const std::filesystem::path& path, const TechDatabase& technology, const LibraryDatabase& library)
{
