...
DatabaseState::DatabaseState(DesignDatabase& borrowed_design) noexcept : _design(&borrowed_design) {}
...
std::unique_ptr<DatabaseState> DatabaseState::open(const Config& config)
{
  ...
  state->_technology = std::make_unique<TechDatabase>(TechDatabaseOptions{.geometry = geometry});
  LefTechImporter(*state->_technology).import(lef_def.lef_files);

  state->_library =
      std::make_unique<LibraryDatabase>(state->_technology->techRegistry(), LibraryDatabaseOptions{.geometry = geometry});
  ...
  state->_owned_design =
      std::make_unique<DesignDatabase>(state->_technology->techRegistry(), state->_library->libraryRegistry());
  state->_design = state->_owned_design.get();
