...
static_assert(!std::is_default_constructible_v<LibraryDatabase>);
static_assert(!std::is_copy_constructible_v<LibraryDatabase>);
static_assert(!std::is_move_constructible_v<LibraryDatabase>);
...
TEST(LibraryDatabaseTest, StoresSeparateSiteAndCellMasterNameDomains)
{
  TechRegistry tech_registry;
  LibraryDatabase database{tech_registry};
...
TEST(LibraryDatabaseTest, UpdatesRecordsAndRejectsDuplicateNamesOrInvalidSiteReferences)
{
  TechRegistry tech_registry;
  LibraryDatabase database{tech_registry};
  auto& sites = database.siteStorage();
...
TEST(LibraryDatabaseTest, RejectsSiteDestructionUntilCellMastersReleaseIt)
{
  TechRegistry tech_registry;
  LibraryDatabase database{tech_registry};
  auto& sites = database.siteStorage();
...
TEST(LibraryDatabaseTest, HandlesStaleIdsAcrossAllCrudOperations)
{
  TechRegistry tech_registry;
  LibraryDatabase database{tech_registry};
  auto& sites = database.siteStorage();
...
TEST(LibraryDatabaseTest, EmbedsHierarchyInObjectComponentsAndPreservesItAcrossUpdates)
{
  TechRegistry tech_registry;
  LibraryDatabase database{tech_registry};
...
TEST(LibraryDatabaseTest, StoresMasterTermsPortsAndUnifiedObsWithTechReferences)
{
  TechRegistry tech_registry;
