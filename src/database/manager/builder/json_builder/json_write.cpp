...
void set_discard(string option){
  ...
  //     IEDALOG.info(ieda::Loc::current(), ele);
  // }
}
                               ...
                               IdbCoordinate<int32_t>* point_2, int32_t width)
{
  ...
  if (point_1->get_y() == point_2->get_y()) {
    ...
    IEDALOG.warn(ieda::Loc::current(), "Error...Regular segment only support horizontal & "
                 "vertical direction... ");
  }
...
int32_t Gds2JsonWrite::write_net_wire_segment_rect(JsonStruct* json_struct, IdbRegularWireSegment* segment)
{
  ...
  if (layer == nullptr) {
    IEDALOG.warn(ieda::Loc::current(), "Error...createNetRect : Layer not exist :  ");
    return kDbFail;
  }
