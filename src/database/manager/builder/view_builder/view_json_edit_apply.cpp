...
bool requireObject(const ViewJson& json, const std::string& context)
{
  ...
  IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " must be an object.");
  return false;
}
...
bool readIntField(const ViewJson& json, const std::string& key, int& value, const std::string& context)
{
  ...
  if (iter == json.end() || !iter->is_number_integer()) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " requires integer field `", key, "`.");
    return false;
  }
  value = iter->get<int>();
  return true;
...
bool readStringField(const ViewJson& json, const std::string& key, std::string& value, const std::string& context)
{
  ...
  if (iter == json.end() || !iter->is_string()) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " requires string field `", key, "`.");
    return false;
  }
...
bool checkStringFieldValue(const ViewJson& json, const std::string& key, const std::string& expected, const std::string& context)
{
  ...
  if (value != expected) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " field `", key, "` must be `", expected, "`.");
    return false;
  }
  return true;
}
...
bool readOptionalStringField(const ViewJson& json, const std::string& key, std::string& value, bool& exists, const std::string& context)
{
  ...
  if (!iter->is_string()) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " field `", key, "` must be a string.");
    return false;
  }
  exists = true;
  value = iter->get<std::string>();
...
bool readPointField(const ViewJson& json, const std::string& key, int32_t& x, int32_t& y, const std::string& context)
{
  ...
  if (iter == json.end() || !iter->is_array() || iter->size() != 2 || !(*iter)[0].is_number_integer() || !(*iter)[1].is_number_integer()) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " requires point field `", key, "` as [x, y].");
    return false;
  }
...
bool readOrientValue(const std::string& orient_name, IdbOrient& orient, const std::string& context)
{
  ...
  if (orient == IdbOrient::kNone) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " has invalid orient `", orient_name, "`.");
    return false;
  }
  return true;
}
...
bool readStatusValue(const std::string& status_name, IdbPlacementStatus& status, const std::string& context)
{
  ...
  if (status == IdbPlacementStatus::kNone && upper_status != "NONE") {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " has invalid status `", status_name, "`.");
    return false;
  }
  return true;
}
...
bool applyMoveInstance(IdbDesign* design, const ViewJson& edit, const std::string& context, bool dry_run)
{
  ...
  if (inst == nullptr) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " references invalid inst_id ", inst_id, ".");
    return false;
  }
...
bool applyMoveIoPin(IdbDesign* design, const ViewJson& edit, const std::string& context, bool dry_run)
{
  ...
  if (pin == nullptr) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " references invalid pin_id ", pin_id, ".");
    return false;
  }
...
bool processEdit(IdbDesign* design, const ViewJson& edit, const std::string& context, bool dry_run)
{
  ...
  IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", context, " has unsupported op `", op, "`.");
  return false;
}
...
bool ViewJsonEditApplier::apply(const std::string& edits_path, bool compressed_hint)
{
  if (_def_service == nullptr || _def_service->get_design() == nullptr) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: def service or design is null.");
    return false;
  }
  ...
  if (!readViewJsonText(edits_path, compressed_hint, content, error)) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: ", error, ".");
    return false;
  }
  ...
  try {
    ...
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: parse ", edits_path, " error: ", error.what());
    return false;
  }
  ...
  if (data_iter == root.end() || !data_iter->is_array()) {
    IEDALOG.warn(ieda::Loc::current(), "Apply view json edits failed: `data` must be an array.");
    return false;
  }
