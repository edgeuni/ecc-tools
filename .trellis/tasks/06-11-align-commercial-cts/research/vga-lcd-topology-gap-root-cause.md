...
| **P0-B** | 表征覆盖修复 | `wirelength_iterations` 自适应 = required_covering_iterations（去 cap）或配置加大；ceil→round/实长表征；评估 runtime 代价 | `direct_bins_capped` 消失；distinct_level_bins ≥ 13；htree 组合中间 buffer 数下降；skew/latency 改善量化 |
| **P1-C** | 拓扑自由度解锁 | 修复 SinkLoadRegion 单调剪枝过度（fanout 检查的边界层语义/per-group 而非 first-fail 整体否决）；让浅深度真正可行；评估非 2^n leaf/混合 fanout-4 结构 | 深度搜索 ≥2 个 feasible 深度；选中深度 buffer 级数 9-10 → ≤8 |
