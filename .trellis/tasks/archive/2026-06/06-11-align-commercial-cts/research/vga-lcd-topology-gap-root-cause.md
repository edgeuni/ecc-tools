...
| **P1-D** | per-branch 时序建模 + skew 修复 | R 修复后：FastSTA 按真实嵌入几何逐分支算 delay（已有 route tree 注入能力）；增加 buffer relocation/插入式 skew 修复或 wire snaking；skew target 自适应 f(period) | **已完成（06-13）**：自适应 target `min(skew_bound, skew_period_fraction×period)` 落地（默认 0.006，vga_lcd 0.08→0.06 对标 Innovus 0.061）；优化器通路探针（target 0.02）实证求解闭环健康（288 scored edits/8 batch trials）且 sizing-only 无法压过 0.0294（级数均一+同 master 后剩余 skew 来自线长离散）→ 插入/蛇形挂起待统一 eval；R4 tolerance 扫描定论保留默认 0.1（tol=10 skew +34.7% 仅省 WL 0.02%，tol=0.02 双输）；spec 增补 skew-target 解析契约。eval 口径条款待统一重跑。详见 `06-11-per-branch-skew-model/research/validation-vga-lcd.md` |
| **P1-E** | Latency 对齐 | 插入延迟缩减阶段（合并 root/trunk 冗余级、leaf 区域 1-to-N 展平、层级跳 buffer 更激进） | **已完成（06-13）**：delay-margin 有界选型（`selection_delay_margin` 默认 0.07，0=回退旧中位）替代三处 Pareto 中位（trunk/per-depth/全局决定点）；trunk 0-buffer 直驱在 strict 过滤中本就幸存、margin>0 即胜出 → **级数 10→8**（trunk 2→0），internal max arrival 0.5564→0.4486（**-19.4%**，外推 eval≈0.503≤0.52）、skew 0.0294→0.0279（改善）、面积 +2.48%/power +2.4%/WL +0%；htree 端 min-delay 仍 5 buffered levels（可行域下限：顶部 fanout 翻倍+底部 split 界），cluster/末级合并经数据判定不立项；root master BUFX20 已饱和。最长 net 434.5→506.7um（max_length 不实施为存量缺陷）。spec 增补 bounded-selection 契约。eval 口径待统一重跑。详见 `06-11-latency-align/research/{structure-levels.md,validation-vga-lcd.md}` |
